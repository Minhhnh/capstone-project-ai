...
from app.ml.modules import devices, sd_hijack_optimizations, shared, sd_hijack_checkpoint
from app.ml.modules.hypernetworks import hypernetwork
from app.ml.modules.shared import cmd_opts
from app.ml.modules import sd_hijack_clip, sd_hijack_open_clip, sd_hijack_unet, sd_hijack_xlmr, xlmr
...
ldm.modules.attention.BasicTransformerBlock.ATTENTION_MODES[
    "softmax-xformers"] = ldm.modules.attention.CrossAttention
...
def apply_optimizations():
    ...
    can_use_sdp = hasattr(torch.nn.functional, "scaled_dot_product_attention") and callable(getattr(
        torch.nn.functional, "scaled_dot_product_attention"))  # not everyone has torch 2.x to use sdp
...
class StableDiffusionModelHijack:
    ...
    embedding_db = app.ml.modules.textual_inversion.textual_inversion.EmbeddingDatabase()
    ...
    def hijack(self, m):
        if type(m.cond_stage_model) == xlmr.BertSeriesModelWithTransformation:
            ...
            m.cond_stage_model = sd_hijack_open_clip.FrozenOpenCLIPEmbedderWithCustomWords(
                m.cond_stage_model, self)
...
def register_buffer(self, name, attr):
    ...
    if type(attr) == torch.Tensor:
        if attr.device != devices.device:
            attr = attr.to(device=devices.device, dtype=(
                torch.float32 if devices.device.type == 'mps' else None))
