...
from app.api.helpers import generation_parameters_copypaste
from app.ml.modules import devices, prompt_parser, masking, sd_samplers, lowvram, extra_networks, sd_vae_approx, scripts
from app.ml.modules.sd_hijack import model_hijack
from app.ml.modules.shared import opts, cmd_opts, state
import app.ml.modules.shared as shared
import app.ml.modules.paths as paths
import app.ml.modules.face_restoration
import app.ml.modules.images as images
import app.ml.modules.styles
import app.ml.modules.sd_models as sd_models
import app.ml.modules.sd_vae as sd_vae
import logging
...
class Processed:
    def __init__(self, p: StableDiffusionProcessing, images_list, seed=-1, info="", subseed=None, all_prompts=None, all_negative_prompts=None, all_seeds=None, all_subseeds=None, index_of_first_image=0, infotexts=None, comments=""):
        ...
        self.subseed = int(self.subseed if type(self.subseed) !=
                           list else self.subseed[0]) if self.subseed is not None else -1
...
def process_images_inner(p: StableDiffusionProcessing) -> Processed:
    ...
    app.ml.modules.sd_hijack.model_hijack.apply_circular(p.tiling)
    app.ml.modules.sd_hijack.model_hijack.clear_comments()
        ...
        for n in range(p.n_iter):
            ...
            for i, x_sample in enumerate(x_samples_ddim):
                ...
                if p.restore_faces:
                    ...
                    x_sample = app.ml.modules.face_restoration.restore_faces(x_sample)
                    devices.torch_gc()
...
class StableDiffusionProcessingTxt2Img(StableDiffusionProcessing):
    ...
    def sample(self, conditioning, unconditional_conditioning, seeds, subseeds, subseed_strength, prompts):
        ...
        if self.enable_hr and latent_scale_mode is None:
            assert len([x for x in shared.sd_upscalers if x.name == self.hr_upscaler]
                       ) > 0, f"could not find upscaler named {self.hr_upscaler}"
        ...
        def save_intermediate(image, index):
            ...
            images.save_image(image, self.outpath_samples, "",
                              seeds[index], prompts[index], opts.samples_format, info=info, suffix="-before-highres-fix")
