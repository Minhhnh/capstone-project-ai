...
from app.ml.modules import shared, devices
...
def split_cross_attention_forward(self, x, context=None, mask=None):
    ...
    dtype = q_in.dtype
    if shared.opts.upcast_attn:
        q_in, k_in, v_in = q_in.float(), k_in.float(
...
def sub_quad_attention_forward(self, x, context=None, mask=None):
    ...
    x = sub_quad_attention(q, k, v, q_chunk_size=shared.cmd_opts.sub_quad_q_chunk_size, kv_chunk_size=shared.cmd_opts.sub_quad_kv_chunk_size,
...
def sub_quad_attention(q, k, v, q_chunk_size=1024, kv_chunk_size=None, kv_chunk_size_min=None, chunk_threshold=None, use_checkpoint=True):
    ...
    if chunk_threshold is None:
        chunk_threshold_bytes = int(get_available_vram(
        ) * 0.9) if q.device.type == 'mps' else int(get_available_vram() * 0.7)
...
def sub_quad_attnblock_forward(self, x):
    ...
    k = k.contiguous()
    v = v.contiguous()
    out = sub_quad_attention(q, k, v, q_chunk_size=shared.cmd_opts.sub_quad_q_chunk_size, kv_chunk_size=shared.cmd_opts.sub_quad_kv_chunk_size,
                             chunk_threshold=shared.cmd_opts.sub_quad_chunk_threshold, use_checkpoint=self.training)
    out = rearrange(out, 'b (h w) c -> b c h w', h=h)
    out = self.proj_out(out)
    return x + out
