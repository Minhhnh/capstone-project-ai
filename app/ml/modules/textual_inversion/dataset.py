...
import random
import tqdm
from app.ml.modules import devices, shared
import re
...
class PersonalizedBase(Dataset):
    def __init__(self, data_root, width, height, repeats, flip_p=0.5, placeholder_token="*", model=None, cond_model=None, device=None, template_file=None, include_cond=False, batch_size=1, gradient_step=1, shuffle_tags=False, tag_drop_out=0, latent_sampling_method='once', varsize=False, use_weight=False):
        ...
        self.image_paths = [os.path.join(data_root, file_path)
        ...
        for path in tqdm.tqdm(self.image_paths):
            ...
            if os.path.exists(text_filename):
                ...
                if re_word:
                    ...
                    filename_text = (
                        shared.opts.dataset_filename_join_string or "").join(tokens)

            npimage = np.array(image).astype(np.uint8)
            npimage = (npimage / 127.5 - 1.0).astype(np.float32)

            torchdata = torch.from_numpy(npimage).permute(
            ...
            latent_sample = model.get_first_stage_encoding(
                latent_dist).squeeze().to(devices.cpu)
            ...
            if use_weight and alpha_channel is not None:
                ...
                weight = torch.tensor(
                    [npweight] * channels).reshape([channels] + latent_size)
            ...
            if latent_sampling_method == "random":
                entry = DatasetEntry(
                    filename=path, filename_text=filename_text, latent_dist=latent_dist, weight=weight)
            else:
                entry = DatasetEntry(
            ...
            if include_cond and not (self.tag_drop_out != 0 or self.shuffle_tags):
                with devices.autocast():
                    entry.cond = cond_model([entry.cond_text]).to(
                        devices.cpu).squeeze(0)
    ...
    def __getitem__(self, i):
        ...
        if self.latent_sampling_method == "random":
            entry.latent_sample = shared.sd_model.get_first_stage_encoding(
                entry.latent_dist).to(devices.cpu)
        return entry
...
class GroupedBatchSampler(Sampler):
    def __init__(self, data_source: PersonalizedBase, batch_size: int):
        ...
        self.probs = [e % batch_size / nrb /
                      batch_size if nrb > 0 else 0 for e in expected]
...
class BatchLoader:
    def __init__(self, data):
        ...
        self.latent_sample = torch.stack(
            [entry.latent_sample for entry in data]).squeeze(1)
