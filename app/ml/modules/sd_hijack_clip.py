...
from app.ml.modules import prompt_parser, devices, sd_hijack
...
class FrozenCLIPEmbedderWithCustomWordsBase(torch.nn.Module):
    ...
    def forward(self, texts):
        ...
        if opts.use_old_emphasis_implementation:
            import app.ml.modules.sd_hijack_clip_old
        ...
        for i in range(chunk_count):
            batch_chunk = [chunks[i] if i < len(
                chunks) else self.empty_chunk() for chunks in batch_chunks]
    ...
    def process_tokens(self, remade_batch_tokens, batch_multipliers):
        ...
        z = z * \
            batch_multipliers.reshape(batch_multipliers.shape + (1,)).expand(z.shape)
        new_mean = z.mean()
...
class FrozenCLIPEmbedderWithCustomWords(FrozenCLIPEmbedderWithCustomWordsBase):
    ...
    def encode_embedding_init_text(self, init_text, nvpt):
        ...
        ids = self.wrapped.tokenizer(
            init_text, max_length=nvpt, return_tensors="pt", add_special_tokens=False)["input_ids"]
