...
parser.add_argument("--data-dir", type=str, default=os.path.dirname(os.path.dirname(
    os.path.realpath(__file__))), help="base path where all user data is stored",)
parser.add_argument("--config", type=str, default=sd_default_config,
...
parser.add_argument("--textual-inversion-templates-dir", type=str, default=os.path.join(
    script_path, 'textual_inversion_templates'), help="directory with textual inversion templates")
parser.add_argument("--hypernetwork-dir", type=str, default=os.path.join(models_path,
                    'hypernetworks'), help="hypernetwork directory")
...
parser.add_argument("--gradio-queue", action='store_true',
                    help="Uses gradio queue; experimental option; breaks restart UI button")
...
def reload_hypernetworks():
    from app.ml.modules.hypernetworks import hypernetwork
    ...
    def do_set_current_image(self):
        ...
        if opts.show_progress_grid:
            self.assign_current_image(
                app.ml.modules.sd_samplers.samples_to_image_grid(self.current_latent))
        else:
            self.assign_current_image(
                app.ml.modules.sd_samplers.sample_to_image(self.current_latent))
...
prompt_styles = app.ml.modules.styles.StyleDatabase(styles_filename)

interrogator = app.ml.modules.interrogate.InterrogateModels("interrogate")
...
options_templates.update(options_section(('interrogate', "Interrogate Options"), {
    ...
    "interrogate_clip_skip_categories": OptionInfo([], "CLIP: skip inquire categories", gr.CheckboxGroup, lambda: {"choices": app.ml.modules.interrogate.category_types()}, refresh=app.ml.modules.interrogate.category_types),
...
mem_mon = app.ml.modules.memmon.MemUsageMonitor("MemMon", device, opts)
mem_mon.start()
